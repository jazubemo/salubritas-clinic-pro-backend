...
export class AppointmentsService {
  ...
  private async find(filters: Record<string, any>): Promise<Appointment[]> {
    try {
      const queryOptions: any = { where: filters };
      ...
      return await this.prisma.appointment.findMany(queryOptions);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.stack : String(error);
      this.logger.error(
